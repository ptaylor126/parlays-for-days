...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(
      `${BASE_URL}/players/topscorers?league=${leagueId}&season=${season}&team=${teamId}`,
      { headers }
    );

    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to fetch top scorers' }, { status: response.status });
    }
