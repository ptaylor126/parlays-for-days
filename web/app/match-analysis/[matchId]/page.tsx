...
type ComparisonRowProps = {
  ...
  rightColor?: string; // Optional override for right side color
};

function ComparisonRow({ left, label, right, showDividers = false, rightColor }: ComparisonRowProps) {
  return (
    <div className="flex items-baseline justify-center py-2 gap-4" style={{ minHeight: "24px" }}>
      ...
      <div className="text-sm font-bold" style={{ color: rightColor || "#90A1B9", minWidth: "40px", textAlign: "left" }}>{right}</div>
    </div>
  );
}
...
function formatTeamName(name: string) {
  ...
  if (name === "Manchester City") {
    return <div style={{ overflowWrap: "break-word", wordBreak: "break-word" }}>Man City</div>;
  }
  // Handle "Manchester United" -> split properly
  if (name === "Manchester United") {
    return (
      <>
        <div style={{ overflowWrap: "break-word", wordBreak: "break-word", maxWidth: "100px" }}>Manchester</div>
        <div style={{ overflowWrap: "break-word", wordBreak: "break-word", maxWidth: "100px" }}>United</div>
      </>
    );
  }
  if (name.length > 12) {
    ...
    if (parts.length >= 2) {
      return (
        <>
          <div style={{ overflowWrap: "break-word", wordBreak: "break-word", maxWidth: "100px" }}>{parts[0]}</div>
          <div style={{ overflowWrap: "break-word", wordBreak: "break-word", maxWidth: "100px" }}>{parts.slice(1).join(" ")}</div>
        </>
      );
    }
  }
  return <div style={{ overflowWrap: "break-word", wordBreak: "break-word" }}>{name}</div>;
}
...
export default function MatchAnalysisPage() {
  ...
  useEffect(() => {
    const fetchMatch = async () => {
      ...
      if (contextMatch) {
        ...
        // Extract fixtureId if available
        const extractedFixtureId = contextMatch.fixtureId || (matchId.includes('api-football-') ? parseInt(matchId.split('api-football-')[1]) : null);
        if (extractedFixtureId) {
          setFixtureId(extractedFixtureId);
        }
        setMatchStartTime(contextMatch.startTime);
  ...
  // Helper to extract fixture ID from matchId
  const getFixtureId = useCallback((id: string): number | undefined => {
    if (id.includes('api-football-')) {
      const extracted = parseInt(id.split('api-football-')[1]);
      return isNaN(extracted) ? undefined : extracted;
    }
    return undefined;
  }, []);

  // Fetch enriched match data when match is loaded
  useEffect(() => {
    if (!match) return;
    ...
    async function fetchAnalysis() {
      try {
        ...
        const matchData = {
          ...
          startTime: matchStartTime || new Date().toISOString(),
        };
        ...
        // Call the match-analysis API route
        const response = await fetch('/api/claude/match-analysis', {
          ...
          body: JSON.stringify({
            matchData,
            fixtureId: fixtureId || getFixtureId(matchId),
            matchStartTime: matchStartTime || matchData.startTime,
          }),
        });
        ...
        console.log('Match data:', enrichedData);
        console.log('Analysis:', analysis);
        
        setEnrichedMatchData(enrichedData);
        console.log('Enriched match data received:', {
          homePosition: enrichedData.homeTeamStats?.position,
          homeForm: enrichedData.homeTeamStats?.form,
          homePoints: enrichedData.homeTeamStats?.points,
          awayPosition: enrichedData.awayTeamStats?.position,
          awayForm: enrichedData.awayTeamStats?.form,
          awayPoints: enrichedData.awayTeamStats?.points,
        });
        
        // Also set analysis for backward compatibility
        if (enrichedData) {
          setAnalysis({
    ...
    fetchAnalysis();
  }, [match, fixtureId, matchStartTime, matchId, getFixtureId]);
        ...
        >
            ...
            <div className="flex items-center gap-3 flex-1 justify-center">
              {(match.homeTeam === "Tottenham" || match.homeTeam === "Tottenham Hotspur" || match.homeTeam === "Spurs") ? (
                <div className="rounded-full flex items-center justify-center" style={{ width: 24, height: 24, backgroundColor: "#FFFFFF", padding: "3px" }}>
                  <Image
        ...
        <main className="px-4 pb-6 space-y-4">
          ...
          <section className="pt-2 pb-6">
            ...
            <div className="flex items-start justify-center gap-2">
              <div className="text-center flex-1 flex flex-col items-center">
                ...
                <div className="text-base font-bold text-white text-center flex flex-col items-center justify-center w-full" style={{ lineHeight: 1.2, wordBreak: "break-word", minHeight: "2.4em", maxWidth: "100px" }}>
                  {formatTeamName(match.homeTeam)}
                </div>
              </div>
              <div className="text-base flex items-start" style={{ color: "#64748b", paddingTop: "48px", marginLeft: "-8px", marginRight: "-8px" }}>vs</div>
              <div className="text-center flex-1 flex flex-col items-center">
                <div className="mb-3 flex items-center justify-center mx-auto flex-shrink-0">
                  {match.awayTeam === "Tottenham" || match.awayTeam === "Tottenham Hotspur" || match.awayTeam === "Spurs" ? (
                    <div
          ...
          <section className="mt-5 mb-8">
            <div className="relative">
              {/* PICK OF THE GAME Badge - overlapping at top */}
              <div
              ...
              <div
                className="rounded-[12px] p-4 pt-6 text-white text-center"
                style={{
              ...
              >
                ...
                <div className="text-base font-semibold">{match.aiPrediction}</div>
                <div className="text-xs opacity-75 mt-2" style={{ color: "#94a3b8" }}>
          ...
          <Section title="Referee Info" icon="/assets/icons/referee.svg">
            <div className="space-y-4">
              <div className="text-lg font-medium text-white">
                {enrichedMatchData?.referee || 'To be confirmed'}
              </div>
              {!enrichedMatchData?.referee && (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center">
                  ...
                  <div className="space-y-2 text-sm">
                    {enrichedMatchData?.homeTeamInjuries && enrichedMatchData.homeTeamInjuries.length > 0 ? (
                      enrichedMatchData.homeTeamInjuries.map((injury: any, idx: number) => (
                        <div key={idx} className="text-white text-xs">
                          {injury.player || injury.name || 'Unknown'} {injury.type || injury.reason ? `(${injury.type || injury.reason})` : ''}
                        </div>
                      ))
                    ) : analysis?.homeTeam?.injuries && analysis.homeTeam.injuries.length > 0 ? (
                      analysis.homeTeam.injuries.map((injury, idx) => (
                        <div key={idx} className="text-white text-xs">
                          {typeof injury === 'string' ? injury : injury.name}
                        </div>
                      ))
                    ) : (
                ...
                <div className="text-center border-l pl-4" style={{ borderColor: "#334155" }}>
                  ...
                  <div className="space-y-3 text-sm">
                    {enrichedMatchData?.awayTeamInjuries && enrichedMatchData.awayTeamInjuries.length > 0 ? (
                      enrichedMatchData.awayTeamInjuries.map((injury: any, idx: number) => (
                        <div key={idx} className="text-white text-xs">
                          {injury.player || injury.name || 'Unknown'} {injury.type || injury.reason ? `(${injury.type || injury.reason})` : ''}
                        </div>
                      ))
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex items-center" style={{ gap: "8px" }}>
                    {enrichedMatchData?.homeTeamStats?.form && Array.isArray(enrichedMatchData.homeTeamStats.form) ? (
                      enrichedMatchData.homeTeamStats.form.map((result, idx) => (
                        <FormIndicator key={idx} result={result as "W" | "L" | "D"} />
                      ))
                    ) : analysis?.homeTeam?.form ? (
                      typeof analysis.homeTeam.form === 'string' 
                        ? analysis.homeTeam.form.split('').map((result, idx) => (
                            <FormIndicator key={idx} result={result as "W" | "L" | "D"} />
                          ))
                        : (analysis.homeTeam.form as string[]).map((result, idx) => (
                        <FormIndicator key={idx} result={result as "W" | "L" | "D"} />
                      ))
                    ) : (
                ...
                <div>
                  ...
                  <div className="flex items-center" style={{ gap: "8px" }}>
                    {enrichedMatchData?.awayTeamStats?.form && Array.isArray(enrichedMatchData.awayTeamStats.form) ? (
                      enrichedMatchData.awayTeamStats.form.map((result, idx) => (
                        <FormIndicator key={idx} result={result as "W" | "L" | "D"} />
                      ))
                    ) : analysis?.awayTeam?.form ? (
                      typeof analysis.awayTeam.form === 'string' 
                        ? analysis.awayTeam.form.split('').map((result, idx) => (
                            <FormIndicator key={idx} result={result as "W" | "L" | "D"} />
                          ))
                        : (analysis.awayTeam.form as string[]).map((result, idx) => (
                        <FormIndicator key={idx} result={result as "W" | "L" | "D"} />
                      ))
                    ) : (
          ...
          <section className="space-y-3">
            ...
            >
              {loadingStats ? (
              ...
              ) : enrichedMatchData?.h2hRecord ? (
                <div className="space-y-2">
                  <ComparisonRow 
                    left={enrichedMatchData.h2hRecord.homeWins.toString()} 
                    label="Wins" 
                    right={enrichedMatchData.h2hRecord.awayWins.toString()} 
                    showDividers={false}
                    rightColor="#FFFFFF"
                  />
                  <ComparisonRow 
                    left={enrichedMatchData.h2hRecord.draws.toString()} 
                    label="Draws" 
                    right={enrichedMatchData.h2hRecord.draws.toString()} 
                    showDividers={false}
                    rightColor="#FFFFFF"
                  />
                </div>
                ...
                <div className="space-y-2">
                  <ComparisonRow left="0" label="Wins" right="0" showDividers={false} />
                  <ComparisonRow left="0" label="Draws" right="0" showDividers={false} />
                </div>
              )}
            </div>
          </section>
          ...
          <Section title="Key Stats" icon="/assets/icons/key-stats.svg">
            {loadingStats ? (
              ...
              <div className="space-y-0">
                {enrichedMatchData?.homeTeamStats && enrichedMatchData?.awayTeamStats ? (
                  <>
                    <ComparisonRow 
                      left={enrichedMatchData.homeTeamStats.goalsFor != null ? Math.floor(enrichedMatchData.homeTeamStats.goalsFor).toString() : 'N/A'} 
                      label="Goals Scored (Total)" 
                      right={enrichedMatchData.awayTeamStats.goalsFor != null ? Math.floor(enrichedMatchData.awayTeamStats.goalsFor).toString() : 'N/A'}
                      rightColor="#FFFFFF"
                    />
                    <ComparisonRow 
                      left={enrichedMatchData.homeTeamStats.position ? enrichedMatchData.homeTeamStats.position + "th" : 'N/A'} 
                      label="League Position" 
                      right={enrichedMatchData.awayTeamStats.position ? enrichedMatchData.awayTeamStats.position + "th" : 'N/A'} 
                    />
                    <ComparisonRow 
                      left={enrichedMatchData.homeTeamStats.points ? enrichedMatchData.homeTeamStats.points + " pts" : 'N/A'} 
                      label="Points" 
                      right={enrichedMatchData.awayTeamStats.points ? enrichedMatchData.awayTeamStats.points + " pts" : 'N/A'} 
                    />
                  </>
                ) : analysis?.homeTeam && analysis?.awayTeam ? (
                  <>
                    <ComparisonRow 
                      left={analysis.homeTeam.goalsFor != null ? Math.floor(analysis.homeTeam.goalsFor).toString() : 'N/A'} 
                      label="Goals Scored (Total)" 
                      right={analysis.awayTeam.goalsFor != null ? Math.floor(analysis.awayTeam.goalsFor).toString() : 'N/A'} 
                      rightColor="#FFFFFF"
                    />
                    <ComparisonRow 
          ...
          {/* Top Scorers */}
          <Section title="Top Scorers" icon="/assets/icons/top-scorers.svg">
            {loadingStats ? (
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                {(enrichedMatchData?.homeTeamStats?.topScorers && enrichedMatchData?.awayTeamStats?.topScorers) ||
                 (analysis?.homeTeam?.scorers && analysis?.awayTeam?.scorers) ? (
                  Array.from({ length: Math.min(
                    Math.max(
                      enrichedMatchData?.homeTeamStats?.topScorers?.length || analysis?.homeTeam?.scorers?.length || 0,
                      enrichedMatchData?.awayTeamStats?.topScorers?.length || analysis?.awayTeam?.scorers?.length || 0
                    ),
                    3 // Show up to 3 top scorers
                  ) }).map((_, idx) => {
                    const homeScorer = enrichedMatchData?.homeTeamStats?.topScorers?.[idx] || analysis?.homeTeam?.scorers?.[idx];
                    const awayScorer = enrichedMatchData?.awayTeamStats?.topScorers?.[idx] || analysis?.awayTeam?.scorers?.[idx];
                    return (
                      <div key={idx} className="flex items-center justify-center" style={{ gap: "8px" }}>
                        <span className="text-sm flex-1 text-white text-right">
                          {homeScorer?.name || '-'}
                        </span>
                        <span className="text-sm font-bold" style={{ color: "#319AE0", minWidth: "20px", textAlign: "right" }}>
                          {homeScorer?.goals || 0}
                        </span>
                        <span style={{ width: "24px" }} />
                        <span className="text-sm font-bold text-white" style={{ minWidth: "20px", textAlign: "left" }}>
                          {awayScorer?.goals || 0}
                        </span>
                        <span className="text-sm flex-1 text-white text-left">
                          {awayScorer?.name || '-'}
                        </span>
                      </div>
                    );
          ...
          {/* Cards */}
          <Section title="Cards" icon="/assets/icons/fouls-cards.svg">
            <div className="text-xs mb-3" style={{ color: "#94a3b8", fontStyle: "italic" }}>
              Average per game
            </div>
            <div className="space-y-0">
              {enrichedMatchData?.homeTeamStats?.yellowCardsAvg !== undefined && enrichedMatchData?.awayTeamStats?.yellowCardsAvg !== undefined ? (
                <>
                  {enrichedMatchData?.homeTeamStats?.foulsAvg !== undefined && enrichedMatchData?.awayTeamStats?.foulsAvg !== undefined ? (
                    <ComparisonRow
                      left={enrichedMatchData.homeTeamStats.foulsAvg.toFixed(1)}
                      label="Fouls"
                      right={enrichedMatchData.awayTeamStats.foulsAvg.toFixed(1)}
                      rightColor="#FFFFFF"
                    />
                  ) : null}
                  <ComparisonRow
                    left={enrichedMatchData.homeTeamStats.yellowCardsAvg.toFixed(1)}
                    label="Yellow"
                    right={enrichedMatchData.awayTeamStats.yellowCardsAvg.toFixed(1)}
                    rightColor="#FFFFFF"
                  />
                  <ComparisonRow
                    left={enrichedMatchData.homeTeamStats.redCardsAvg.toFixed(1)}
                    label="Red"
                    right={enrichedMatchData.awayTeamStats.redCardsAvg.toFixed(1)}
                    rightColor="#FFFFFF"
                  />
                </>
              ) : (
          ...
          <Section title="Advanced Stats" icon="/assets/icons/advanced-stats.svg">
            ...
            <div className="space-y-0">
              {enrichedMatchData?.homeTeamStats?.shotsAvg !== undefined && enrichedMatchData?.awayTeamStats?.shotsAvg !== undefined ? (
                <>
                  {enrichedMatchData.homeTeamStats.shotsAvg !== 0 || enrichedMatchData.awayTeamStats.shotsAvg !== 0 ? (
                    <ComparisonRow
                      left={enrichedMatchData.homeTeamStats.shotsAvg.toFixed(1)}
                      label="Shots"
                      right={enrichedMatchData.awayTeamStats.shotsAvg.toFixed(1)}
                      rightColor="#FFFFFF"
                    />
                  ) : null}
                  {enrichedMatchData.homeTeamStats.shotsOnTargetAvg !== 0 || enrichedMatchData.awayTeamStats.shotsOnTargetAvg !== 0 ? (
                    <ComparisonRow
                      left={enrichedMatchData.homeTeamStats.shotsOnTargetAvg.toFixed(1)}
                      label="On Target"
                      right={enrichedMatchData.awayTeamStats.shotsOnTargetAvg.toFixed(1)}
                      rightColor="#FFFFFF"
                    />
                  ) : null}
                  {enrichedMatchData.homeTeamStats.cornersAvg !== 0 || enrichedMatchData.awayTeamStats.cornersAvg !== 0 ? (
                    <ComparisonRow
                      left={enrichedMatchData.homeTeamStats.cornersAvg.toFixed(1)}
                      label="Corners"
                      right={enrichedMatchData.awayTeamStats.cornersAvg.toFixed(1)}
                      rightColor="#FFFFFF"
                    />
                  ) : null}
                  {enrichedMatchData.homeTeamStats.xGAvg !== undefined && enrichedMatchData.homeTeamStats.xGAvg !== null &&
                   (enrichedMatchData.homeTeamStats.xGAvg !== 0 || enrichedMatchData.awayTeamStats.xGAvg !== 0) ? (
                    <ComparisonRow
                      left={enrichedMatchData.homeTeamStats.xGAvg.toFixed(1)}
                      label="xG"
                      right={enrichedMatchData.awayTeamStats.xGAvg?.toFixed(1) || '0.0'}
                      rightColor="#FFFFFF"
                    />
                  ) : null}
                </>
