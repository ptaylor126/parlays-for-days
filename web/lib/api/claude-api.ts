...
export async function analyzeSingleMatch(match: Match) {
  ...
  const seasonString = `${seasonYear}/${nextYear.toString().slice(-2)}`;
...
IMPORTANT: You MUST use data from the ${seasonString} season (${seasonYear}-${nextYear} season). Do NOT use data from previous seasons.

Match: ${match.homeTeam} vs ${match.awayTeam}
League: ${match.league}
Season: ${seasonString} (${seasonYear}-${nextYear})
Current Date: ${today.toISOString().split('T')[0]}
Odds: Home ${match.odds.home} | Draw ${match.odds.draw} | Away ${match.odds.away}

Review recent form, head-to-head, injuries, and key stats from the ${seasonString} season ONLY. Provide betting recommendations including winners, goal scorers, both teams to score, over/under, etc.
...
3. All statistics (position, points, goals, form, scorers) MUST be from the ${seasonString} season, NOT from previous seasons
4. If uncertain about data, use null or reasonable estimates (never use "unavailable" as a string)
5. All confidence percentages must be realistic (30-85%)
6. Include 5-6 different betting angles
7. Form should be 5 characters (W/L/D only) representing the last 5 matches from the ${seasonString} season
8. Top scorers data MUST be from the ${seasonString} season only
...
Return this EXACT JSON structure:
{
  "homeTeam": {
    ...
    "injuries": [
      {"name": "Player Name", "reason": "Injury type"}
    ],
  ...
  try {
    ...
    return {
      homeTeam: {
        ...
        injuries: [{ name: 'Data unavailable', reason: 'Analysis unavailable' }],
